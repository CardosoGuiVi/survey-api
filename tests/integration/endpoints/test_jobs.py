...
async def test_list_jobs(client: AsyncClient) -> None:
    ...
    response = await client.get("/v1/org/jobs")
...
async def test_get_job(client: AsyncClient) -> None:
    ...
    response = await client.get(f"/v1/org/jobs/{job_id}")
    assert response.status_code == 200
...
async def test_get_job_not_found(client: AsyncClient) -> None:
    response = await client.get("/v1/org/jobs/00000000-0000-0000-0000-000000000000")
...
async def test_delete_job(client: AsyncClient) -> None:
    ...
    job_id = created.json()["id"]
    response = await client.delete(f"/v1/org/jobs/{job_id}")
